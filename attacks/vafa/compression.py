...
######################## 2D or 3D ########################
def quantize(dct_image, q_table, alpha):
    """ Quantize the DCT coefficients with q_table
    Input:
        dct_image : [B, C, N_Blocks, Block_H, Block_W, Block_D]
    ...
    pre_q_dct_image = dct_image/(q_table)                 # quatization of DCT coefficients
    after_q_dct_image = phi_diff(pre_q_dct_image, alpha)  # differentiable rounding method

    # after_q_dct_image = torch.round(pre_q_dct_image) # to check how standard rounding works in terms of attack accuracy
