...
class SSA_2D(Attack):
    ...
    def __init__(self, model,loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, steps=10, rho=0.5, eps=16/255.0, num_spectrum_augs=10, block_size=(96,96), targeted=False, verbose=True):
        ...
        # number of frequency-domain augmentations 
        self.num_spectrum_augs = num_spectrum_augs

        self.rho = rho
        self.eps = eps
        self.sigma = eps        # SSA recommends to use sigma=eps
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            noise = 0
            for j in range(self.num_spectrum_augs):
                ...
                images_tilde = merged_blocks/255.0 # [B,C,D,H,W]
                images_tilde = images_tilde.permute(0,1,3,4,2) # [B,C,D,H,W] -->  [B,C,H,W,D]

                images_tilde = V(images_tilde, requires_grad = True)

                logits = self.model(images_tilde) # logits: [B,NumClasses,H,W,D] , passing adversarial images through the model 
                ...
                noise = noise + images_tilde.grad.data


            noise = noise/self.num_spectrum_augs        
